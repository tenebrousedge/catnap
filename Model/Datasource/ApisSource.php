...
App::uses('HttpSocketOauth', 'Catnap.Network/Http');
 ...
 * @var string
 */
	public $description = 'CouchDb DataSource';
 ...
 */
	public function describe(\Model $model) {
		//TODO: Rewrite this to do something appropriate
 ...
 */
	protected function _buildRequest($apiName, $type = 'read', $request = array()) {
		if (empty($this->map)) {
			$this->map = Configure::read("Catnap.$apiName.path");
		}
		$host = Configure::read("Catnap.$apiName.Api");
		if (is_array($host)) {
			$this->setConfig($host);
		}
 ...
 */
	protected function _scanMap($action, $section, $fields = array()) {
		...
		if (!Hash::check($map, $section)) {
			throw new CakeException(__('Section %s not found in Catnap Driver Configuration Map - ', $section) . get_class($this), 500);
		} else {
